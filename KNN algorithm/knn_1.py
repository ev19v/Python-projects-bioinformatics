...
class KNNClassifier:
    ...
    def predict_labels_binary(self, distances):
        ...
        n_train_samples = distances.shape[1]
        n_test_samples = distances.shape[0]
        prediction = []

        closest_ids = np.argsort(distances, axis=1)[:, :self.k]
        closest = self.train_y[closest_ids]

        for i in range(n_test_samples):
            values, counts = np.unique(closest[i], return_counts=True)
            ind = np.argmax(counts)
            prediction.append(np.array(values[ind]))
        return prediction
    ...
    def predict_labels_multiclass(self, distances):
        ...
        n_train_samples = distances.shape[1]
        n_test_samples = distances.shape[0]
        prediction = np.zeros(n_test_samples)
